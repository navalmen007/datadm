...
class StarChat:
    def __init__(self, model_path=None, revision=None, **kwargs):
        import torch
        self.tokenizer = AutoTokenizer.from_pretrained(model_path, device_map='auto', revision=revision)
        self.model = AutoModelForCausalLM.from_pretrained(model_path, device_map='auto', torch_dtype=torch.bfloat16, revision=revision)
        self.model.eval()
